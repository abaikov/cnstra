...
export const PerformanceMonitor: React.FC = () => {
    ...
    const getCNSMetrics = () => {
        ...
        const stimulationsPerSecond = timeDiff > 0 ? newStimulations / timeDiff : 0;

        // Calculate average response time
        const recentResponses = allResponses.filter(r => r.duration && r.duration > 0);
        const avgResponseTime = recentResponses.length > 0
            ? recentResponses.reduce((sum, r) => sum + (r.duration || 0), 0) / recentResponses.length
            : 0;
        ...
        const recentStimulations = allStimulations.filter(s => s.timestamp > recentTime);
        const activeNeuronIds = new Set(recentStimulations.map(s => s.neuronId));
        ...
        const errorRate = allResponses.length > 0 ? (responsesWithErrors.length / allResponses.length) * 100 : 0;

        return {
            stimulationsPerSecond: Math.round(stimulationsPerSecond * 100) / 100,
    ...
    useEffect(() => {
        const updateMetrics = () => {
            ...
            setHistory(prev => {
                const maxPoints = 60;
                const newHistory = {
                    memory: [...prev.memory, memoryUsage.percentage].slice(-maxPoints),
                    timestamps: [...prev.timestamps, newMetrics.timestamp].slice(-maxPoints),
                    stimulationsPerSecond: [...prev.stimulationsPerSecond, cnsMetrics.stimulationsPerSecond].slice(-maxPoints),
                    responseTime: [...prev.responseTime, cnsMetrics.avgResponseTime].slice(-maxPoints),
                    hopCount: [...prev.hopCount, cnsMetrics.avgHopCount].slice(-maxPoints),
                    errorRate: [...prev.errorRate, cnsMetrics.errorRate].slice(-maxPoints)
                };
                return newHistory;
            });
        };
    ...
    const memoryColor = getStatusColor(metrics.memoryUsage.percentage, { warning: 70, danger: 90 });
            ...
            {!isExpanded ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-sm)' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--spacing-xs)', fontSize: '10px' }}>
                            <div>
                                <span style={{ color: 'var(--text-muted)' }}>⚡ Stimulations/s:</span>
                                <br />
                                <span style={{ color: 'var(--infection-green)' }}>{metrics.cnsMetrics.stimulationsPerSecond}</span>
                            </div>
                            <div>
                                <span style={{ color: 'var(--text-muted)' }}>🎯 Active Neurons:</span>
                                <br />
                                <span style={{ color: 'var(--infection-blue)' }}>{metrics.cnsMetrics.activeNeurons}</span>
                            </div>
                            <div>
                                <span style={{ color: 'var(--text-muted)' }}>⏱️ Avg Response:</span>
                                <br />
                                <span style={{ color: metrics.cnsMetrics.avgResponseTime > 100 ? 'var(--infection-red)' : 'var(--infection-green)' }}>
                            ...
                            <div>
                                ...
                                <br />
                                <span style={{ color: 'var(--text-primary)' }}>{metrics.cnsMetrics.totalConnections}</span>
                            </div>
                            <div>
                                <span style={{ color: 'var(--text-muted)' }}>🎭 Avg Hops:</span>
                                <br />
                                <span style={{ color: metrics.cnsMetrics.avgHopCount > 3 ? 'var(--infection-yellow)' : 'var(--infection-green)' }}>
                                    {metrics.cnsMetrics.avgHopCount}
                                </span>
                            </div>
                            <div>
                                <span style={{ color: 'var(--text-muted)' }}>📊 Queue Util:</span>
                                <br />
                                <span style={{ color: metrics.cnsMetrics.queueUtilization > 10 ? 'var(--infection-red)' : 'var(--infection-green)' }}>
                    ...
                    {history.stimulationsPerSecond.length > 1 && (
                        <div>
                            <div style={{
                                ...
                                📈 Stimulations Trend (last {history.stimulationsPerSecond.length}s)
                            </div>
                            <div style={{
                                height: '15px',
                                background: 'var(--bg-secondary)',
                                ...
                                >
                                    <polyline
                                        ...
                                        strokeWidth="1"
                                        points={history.stimulationsPerSecond.map((value, index) => {
                                            const maxValue = Math.max(...history.stimulationsPerSecond, 1);
                                            return `${(index / (history.stimulationsPerSecond.length - 1)) * 100},${15 - (value / maxValue) * 15}`;
                                        }).join(' ')}
                                    />
                                </svg>
                            </div>
                            ...
                            <div style={{
                                ...
                                >
                                    <polyline
                                        ...
                                        stroke={memoryColor}
                                        strokeWidth="1"
                                        points={history.memory.map((value, index) =>
                                            `${(index / (history.memory.length - 1)) * 100},${15 - (value / 100) * 15}`
                                        ).join(' ')}
                                    />
                                </svg>
                            </div>
